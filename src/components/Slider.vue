  ...
  <div class="v_slider">
    ...
    <!--
        <ul v-if="dots" class="v_slider__dots">
            ...
            <li v-for="n in numDot" @click="setDot(n)" :class="{'active': n == numDotActive}" v-bind:key="n">
                <span></span>
            </li>
        </ul>
-->
      <button v-if="nav" class="v_slider__prev" @click="prevSlide"></button>
      <button v-if="nav" class="v_slider__next" @click="nextSlide"></button>
  </div>
</template>
    ...
    methods: {
      ...
      getWidth() {
        ...
        if (this.settings.items == 1) {
          ...
          this.width.slide = (this.width.container / this.settings.items) - ((this.settings.items - 1) * (this.settings
              .margin) / this.settings.items),
